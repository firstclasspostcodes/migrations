...
const {
  createMigration, createMigrationsTable, getMigrations,
} = require('./sql');
const {
  readFile, fileChecksum, fileTimestamp, mapColumns,
} = require('./utils');
...
exports.executeMigration = async (dataApiClient, migration, params = {}) => (
  dataApiClient.executeStatement({
    sql: readFile(migration.filepath),
    ...params,
  }).promise()
);
